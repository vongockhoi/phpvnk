...
class ProductCategoryController extends Controller
{
    ...
    public function store(StoreProductCategoryRequest $request)
    {
        ...
        if ($request->input('icon', false)) {
            $productCategory->addMedia(storage_path('tmp/uploads/' . basename($request->input('icon'))))->toMediaCollection('icon');
        }

        if ($request->input('avatar', false)) {
            $productCategory->addMedia(storage_path('tmp/uploads/' . basename($request->input('avatar'))))->toMediaCollection('avatar');
        }
    ...
    public function update(UpdateProductCategoryRequest $request, ProductCategory $productCategory)
    {
        ...
        if ($request->input('icon', false)) {
            if (!$productCategory->icon || $request->input('icon') !== $productCategory->icon->file_name) {
                if ($productCategory->icon) {
                    $productCategory->icon->delete();
                }
                $productCategory->addMedia(storage_path('tmp/uploads/' . basename($request->input('icon'))))->toMediaCollection('icon');
            }
        ...
        if ($request->input('avatar', false)) {
            if (!$productCategory->avatar || $request->input('avatar') !== $productCategory->avatar->file_name) {
                if ($productCategory->avatar) {
                    $productCategory->avatar->delete();
                }
                $productCategory->addMedia(storage_path('tmp/uploads/' . basename($request->input('avatar'))))->toMediaCollection('avatar');
            }
        } elseif ($productCategory->avatar) {
            $productCategory->avatar->delete();
        }
