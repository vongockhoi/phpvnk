...
class BannerController extends Controller
{
    ...
    public function store(StoreBannerRequest $request)
    {
        ...
        if ($request->input('avatar', false)) {
            $banner->addMedia(storage_path('tmp/uploads/' . basename($request->input('avatar'))))->toMediaCollection('avatar');
        }
    ...
    public function update(UpdateBannerRequest $request, Banner $banner)
    {
        ...
        if ($request->input('avatar', false)) {
            if (!$banner->avatar || $request->input('avatar') !== $banner->avatar->file_name) {
                if ($banner->avatar) {
                    $banner->avatar->delete();
                }
                $banner->addMedia(storage_path('tmp/uploads/' . basename($request->input('avatar'))))->toMediaCollection('avatar');
            }
        } elseif ($banner->avatar) {
            $banner->avatar->delete();
        }
