    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.products.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('brief_description') ? 'is-invalid' : '' }}" type="text" name="brief_description" id="brief_description" value="{{ old('brief_description', '') }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('hash_tags') ? 'is-invalid' : '' }}" name="hash_tags[]" id="hash_tags" multiple>
                    @foreach($hash_tags as $id => $hash_tag)
                        <option value="{{ $id }}" {{ in_array($id, old('hash_tags', [])) ? 'selected' : '' }}>{{ $hash_tag }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('restaurants') ? 'is-invalid' : '' }}" name="restaurants[]" id="restaurants" multiple>
                    @foreach($restaurants as $id => $restaurant)
                        <option value="{{ $id }}" {{ in_array($id, old('restaurants', [])) ? 'selected' : '' }}>{{ $restaurant }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <label for="restaurants">{{ trans('cruds.product.fields.category') }}</label>
                <div style="padding-bottom: 4px">
                    <span class="btn btn-info btn-xs select-all" style="border-radius: 0">{{ trans('global.select_all') }}</span>
                    <span class="btn btn-info btn-xs deselect-all" style="border-radius: 0">{{ trans('global.deselect_all') }}</span>
                </div>
            ...
            <div class="form-group">
                ...
                <div style="display: flex;">
                    <input class="form-control {{ $errors->has('preparation_time_hour') ? 'is-invalid' : '' }}" style="width: 60px;" type="number" name="preparation_time_hour" id="preparation_time_hour" value="{{ old('preparation_time_hour', '0') }}" step="1">
                    <span style="line-height: 30px;">:</span>
                    <input class="form-control {{ $errors->has('preparation_time_minute') ? 'is-invalid' : '' }}" style="width: 100px;" type="number" name="preparation_time_minute" id="preparation_time_minute" value="{{ old('preparation_time', '0') }}" step="1">
                </div>
            ...
            <div class="form-group form-currency">
                ...
                <input class="form-control currency_price {{ $errors->has('price') ? 'is-invalid' : '' }}" type="text" name="price" id="price" value="{{ old('price', '0') }}" step="0.01" required>
            ...
            <div class="form-group form-currency">
                ...
                <input class="form-control currency_price_discount {{ $errors->has('price_discount') ? 'is-invalid' : '' }}" type="text" name="price_discount" id="price_discount" value="{{ old('price_discount') }}" step="0.01">
                <span class="currency">đ</span>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('is_price_change') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="is_price_change" id="is_price_change" value="1" {{ old('is_price_change', 0) == 1 ? 'checked' : '' }}>
...
{{--                    <option value="1" {{ in_array(1, old('type', [])) ? 'selected' : '' }}>Bán theo phần</option>--}}
{{--                    <option value="2" {{ in_array(2, old('type', [])) ? 'selected' : '' }}>Bán theo con</option>--}}
{{--                    <option value="3" {{ in_array(3, old('type', [])) ? 'selected' : '' }}>Bán theo kg</option>--}}
{{--                </select>--}}
{{--                @if($errors->has('type'))--}}
{{--                    <div class="invalid-feedback">--}}
{{--                        {{ $errors->first('type') }}--}}
...
{{--                <span class="help-block">{{ trans('cruds.product.fields.type_helper') }}</span>--}}
                ...
                <select class="form-control select2 {{ $errors->has('product_unit_id') ? 'is-invalid' : '' }}" name="product_unit_id" id="product_unit_id">
                    @foreach($product_units as $id => $entry)
                        <option value="{{ $id }}" {{ old('product_unit_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
