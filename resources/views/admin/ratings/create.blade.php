    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.ratings.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('order') ? 'is-invalid' : '' }}" name="order_id" id="order_id" required>
                    @foreach($orders as $id => $entry)
                        <option value="{{ $entry }}" {{ old('order_id') == $entry ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('order'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('point_rating') ? 'is-invalid' : '' }}" type="number" name="point_rating" id="point_rating" value="{{ old('point_rating', '') }}" step="1" required>
