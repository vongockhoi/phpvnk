        ...
        <div class="card-body">
            <form method="POST" action="{{ route("admin.notifications.store") }}" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control {{ $errors->has('target_type') ? 'is-invalid' : '' }}"
                            name="target_type" id="target_type" required>
                        <option value
                                disabled {{ old('target_type', null) === null ? 'selected' : '' }}>{{ trans('global.pleaseSelect') }}</option>
                        @foreach(App\Models\Notification::TARGET_TYPE_SELECT as $key => $label)
                            <option value="{{ $key }}" {{ old('target_type', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
...
{{--                    <select class="form-control select2 {{ $errors->has('target') ? 'is-invalid' : '' }}"--}}
{{--                            name="target_id" id="target_id">--}}
...
{{--                </div>--}}
                <div class="form-group">
                    ...
                    <input class="form-control datetime {{ $errors->has('schedule_time') ? 'is-invalid' : '' }}"
                           type="text" name="schedule_time" id="schedule_time" value="{{ old('schedule_time') }}">
...
@section('scripts')
{{--    <script>--}}
{{--        var tmp = {!! json_encode($targets->toArray()) !!};--}}
{{--        $( ".optional" ).hide();--}}
