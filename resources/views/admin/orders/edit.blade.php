        ...
        <form method="POST" action="{{ route("admin.orders.update", [$order->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('restaurant') ? 'is-invalid' : '' }}" name="restaurant_id" id="restaurant_id" required>
                    @foreach($restaurants as $id => $entry)
                        <option value="{{ $id }}" {{ (old('restaurant_id') ? old('restaurant_id') : $order->restaurant->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('is_prepay') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="is_prepay" id="is_prepay" value="1" {{ $order->is_prepay || old('is_prepay', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('is_delivery') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="is_delivery" id="is_delivery" value="1" {{ $order->is_delivery || old('is_delivery', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('coupon_customer') ? 'is-invalid' : '' }}" name="coupon_customer_id" id="coupon_customer_id">
                    @foreach($coupon_customers as $id => $entry)
                        <option value="{{ $id }}" {{ (old('coupon_customer_id') ? old('coupon_customer_id') : $order->coupon_customer->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
